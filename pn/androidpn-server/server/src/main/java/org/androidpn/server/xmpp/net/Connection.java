...
public class Connection {
    ...
    private void deliverRawText(String text, boolean asynchronous) {
        ...
        if (!isClosed()) {
            ...
            try {
                ...
                ByteBuf buffer = Unpooled.buffer(tx.length);
                ...
                if (asynchronous) {
                    ...
                    boolean ok = channel.writeAndFlush(buffer).awaitUninterruptibly(
    ...
    public void startTLS(ClientAuth authentication) throws Exception {
        ...
        channel.pipeline().addFirst("tls", new SslHandler(tlsContext.createSSLEngine()));
