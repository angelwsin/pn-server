...
import org.androidpn.server.util.XmppTag;
...
public class StanzaHandler {
    ...
    public void process(String stanza, XMPPPacketReader reader)
        ...
        if (XmppTag.starttls.name().equals(tag)) {
        ...
        } else if (XmppTag.message.name().equals(tag)) {
            processMessage(doc);
        } else if (XmppTag.presence.name().equals(tag)) {
            log.debug("presence...");
            processPresence(doc);
        } else if (XmppTag.iq.name().equals(tag)) {
            log.debug("iq...");
            processIQ(doc);
        } else {
