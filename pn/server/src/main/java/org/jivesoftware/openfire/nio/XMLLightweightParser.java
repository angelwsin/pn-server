...
public class XMLLightweightParser {
    ...
    */
    public void read(ByteBuf byteBuffer) throws Exception {
        ...
        if (byteBuffer.capacity() > maxBufferSize) {
            throw new Exception("Stopped parsing never ending stanza");
        }
        CharBuffer charBuffer = encoder.decode(byteBuffer.nioBuffer());
        char[] buf = charBuffer.array();
        // int readByte = charBuffer.remaining();
        int readByte = byteBuffer.readableBytes();
       ...
       /* buffer.append(buf, 0, readByte);
        ...
        final int n = byteBuffer.writerIndex();
        for (int i = byteBuffer.readerIndex();i<n;i++) {
            ch =(char) byteBuffer.readByte();
            ...
            if (status == XMLLightweightParser.TAIL) {
                ...
                if (depth < 1 && ch == head.charAt(tailCount)) {
                    ...
                    if (tailCount == head.length()) {
                        ...
                        int end = byteBuffer.readerIndex() ;
                       // String msg = buffer.substring(startLastMsg, end);
                        // Add message to the list
                        foundMsg(msg(startLastMsg,end,byteBuffer));
                        startLastMsg = end;
                    }
                } else {
                    tailCount = 0;
                ...
                if (ch == '>') {
                    ...
                    if (depth < 1) {
                        ...
                        int end = byteBuffer.readerIndex();
                        //String msg = new String(buf);buffer.substring(startLastMsg, end);
                        // Add message to the list
                        foundMsg(msg(startLastMsg,end,byteBuffer));
                        startLastMsg = end;
                    }
                ...
                if (ch == '"') {
                    ...
                    if (insideRootTag
                        ...
                        while (startLastMsg < end
                                && '<' != byteBuffer.getByte(startLastMsg)) {
                            startLastMsg++;
                        }
        ...
        byteBuffer.readerIndex(startLastMsg);
        if (head.length() > 0
    ...
    private String  msg(int start,int end,ByteBuf buf)throws Exception{
        byte[] dst = new byte[end-startLastMsg];
        buf.getBytes(startLastMsg, dst, 0, end-startLastMsg);
